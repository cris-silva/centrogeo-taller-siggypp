...
El primer paso es iniciar un servidor de *Selenium*, existen varios métodos como puedes leer en la [documentación de *RSelenium*](https://docs.ropensci.org/RSelenium/articles/basics.html#do-i-need-to-run-a-selenium-server)*,* en ésta práctica, ejecutaremos el archivo binario de *Selenium Server (Grid)* para *Java.*

Descarga el archivo de la versión más reciente de *Selenium Server (Grid)* desde su [página oficial de descargas](https://www.selenium.dev/downloads/). Guarda una copia del archivo `selenium-server-x.xx.x.jar` en la raíz de la carpeta de trabajo.
...
Inicia el servidor de Selenium desde la pestaña *Terminal* de *RStudio*, con la siguiente instrucción (usaremos como ejemplo la versión 4.20.0, cambia el número de la versión si es necesario):

``` bash
java -jar selenium-server-4.20.0.jar standalone --port 4445
```

Por defecto, *Selenium* usa el puerto 4444. Observa que en la instrucción anterior se usa el parámetro `--port 4445` para usar el puerto 4445 en su lugar, ya que de acuerdo con la [documentación de *RSelenium*](https://docs.ropensci.org/RSelenium/articles/basics.html#how-do-i-run-the-selenium-server), se pueden presentar problemas en sistemas con *macOS*. Si trabajas en un entorno *Windows* puedes omitir esta modificación y trabajar con el puerto por defecto.

El servidor *Selenium* ya está activo si después de ejecutar la instrucción anterior aparece en la última línea un mensaje similar a este:

```         
14:58:50.691 INFO [Standalone.execute] - Started Selenium Standalone 4.20.0 (revision 866c76ca80): http://192.168.20.251:4445
```

Para comenzar a enviar instrucciones a *Selenium* para que controle a *Firefox*, debemos definir qué explorador vamos a manejar. Usa la función `rsDriver` y especifica los parámetros `browser = "firefox"` para indicar a *Selenium* que use *Firefox, `port = 4567L`* para indicar con número entero el puerto que se utilizará para recibir las instrucciones y `chromever = NULL, iedrver = NULL, phantomver = NULL` para evitar que se carguen los controladores de los otros exploradores y evitar posibles errores. Guarda la *instancia del servidor* en la variable `servidor_selenium`:

```{r Iniciar servidor de Selenium}
servidor_selenium <- remoteDriver(remoteServerAddr = "192.168.20.251",
                                  port = 4444L,
                                  browserName = "firefox")
```

Después de esta instrucción, *Firefox* debe abrirse automáticamente. A continuación, es necesario conectar el "control remoto" de *R* al servidor de *Selenium* que acabas de iniciar. Para ello, crea el `controlador` a partir del elemento de lista `[["client"]]` de la variable `servidor_selenium` con la que arrancaste el servidor de *Selenium* en el paso anterior*.*
...
```{r}
servidor_selenium$getStatus()
servidor_selenium$open()
```
