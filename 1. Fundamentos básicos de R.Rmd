...
Sigue las indicaciones y ejecuta los siguientes *chunks* para obtener el resultado. En los casos necesarios, completa el código escribiendo sobre las líneas (___) y ejecuta el *chunk* para comprobar el resultado.  
...
Completa el siguiente bloque para obtener la resta de `45 - 13` (sustituye la línea ___ con el valor necesario):
```{r Resta}
45 - ___
```
...
mi_texto <- ___
___ # Visualizar el valor de la variable
```
...
a <- as.integer(___) # Cuando se convierte el número a entero se trunca la parte decimal
___
```
...
```{r Crear una lista}
otro_vector <- c("A", ___, ___) # Crear otro vector con valores
mi_lista <- list(TRUE, ___, ___) # Construir la lista
___ # Visualizar el contenido de la lista
```
...
```{r Crear otra lista}
otra_lista <- ___(___, ___)
___ # Previsualiza el contenido de la lista
```
...
```{r Crear un factor}
a <- factor(c("bajo", "bajo", ___, ___, ___, ___, ___))
___
```
...
rep(___, times = 4) # Repetir un vector n veces
```
...
rep(___, each = 3) # Repetir cada valor de un vector n veces
```
...
sort(___, decreasing = ___) # Orden descendente
```
...
___[4] # Selecciona el 4to. elemento en mi_vector
```
...
___[-4] # Selecciona todo excepto el 4to. elemento en mi_vector
```

Selecciona los elementos 2 al 5 de `mi_vector`:
```{r Selecciona por rango}
___[___:5] # Selecciona del 2do. al 5to. elemento en mi_vector
```
...
___[-(2:___)] # Selecciona todos los elementos excepto los elementos del 2do. al 5to. en mi_vector
```

Selecciona sólo los elementos 1 y 5 de `mi_vector` usando el vector `c(1,5)`:
```{r Seleccionar por índices en particular}
___[c(___,5)] # Selecciona sólo el 1ro. y 5to. elementos en mi_vector
```

Seleccionar elementos de `mi_vector` que son iguales a 2:
```{r Seleccionar por valor}
___[mi_vector == ___] # Seleccionar elementos iguales a 2 en mi_vector
```

Selecciona elementos de `mi_vector` que sean menores a 3:
```{r Seleccionar con una condición}
___[___ < 3] # Selecciona los elementos menores a 3 en mi_vector
```
...
mi_vector > ___ # Comparar cada elemento en mi_vector con la variable a
```

Verificar si el valor de `b` está entre los elementos de `mi_vector` usando el operador `%in%`:
```{r}
b %in% ___ # b es un elemento de mi_vector?
```
...
```{r Crea un dataframe}
mi_df <- data.frame(x = ___, y = c(___))
___ # Visualizar el contenido del dataframe 
```
...
head(___, n = ___) # Si se omite el parámetro n, la función head regresa las primeras 5 filas por defecto
```
...
mi_df[___, ] # Omite el índice de la columna para acceder a la fila completa
```
...
mi_df[, ___] # Omite el índice de la fila para acceder a la columna completa
```
...
mi_df$___ # Usa el nombre de la variable
```
...
nueva_columna <- data.frame(___ = c(___:___)) # Debe tener el mismo número de filas que el dataframe
___
```

Agrega la columna del dataframe `nueva_columna` a `mi_df` usando la función `cbind`:
```{r Agregar columna a un dataframe}
mi_df <- cbind(mi_df, ___)
___
```

Extrae la fila 2 de `mi_df`, asígnala a la variable `nueva_fila` y visualiza su contenido:
```{r Crear nueva fila para dataframe}
nueva_fila <- mi_df[___, ] # Agregar una copia de la fila 2, debe tener los mismos nombres de variables o se agregarán nuevas columnas con los nombres restantes
___ # Visualiza el contenido del dataframe
```

Agregar la fila de `nueva_fila` al `mi_df`usando la función `rbind`:
```{r Agregar fila al dataframe}
mi_df <- rbind(mi_df, ___)
___
```
...
```{r Bucle while}
i <- ___ # Valor inicial de i, si no se especifica correctamente el ciclo se puede ejecutar infinitamente
while(i < ___) {
  print(___)
  i <- i + ___
}
...
```{r}
# Llama la función:
calcular_porcentaje(parte = ___, total = ___)
```
...
save(mi_vector, ___, ___, file = "Datos/mis_datos.RData")
```
...
write.table(___, file = ___, sep = ___) # Guarda el dataframe como texto delimitado usando "|" como separador
```
...
write.csv(___, file = ___) # Guarda el dataframe como CSV usando "," como separador
```
...
write.csv2(___, file = ___) # Guarda el dataframe como CSV usando ";" como separador (para países donde el punto decimal es ",")
```
...
mi_df_desde_delimitado <- read.table(___, sep = ___)
...
__(___ = ___()) # Elimina todas las variables
```
...
data(___, package = "carData") # Carga los datos de Salaries del paquete carData y guárdalo en una variable
___ # Visualiza los datos
```
