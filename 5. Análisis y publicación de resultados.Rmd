...
Para identificar las zonas de rezago, será necesario que uses la información sobre grados de rezago social por AGEB (GRS) publicada por el **Consejo Nacional de Evaluación de la Política de Desarrollo Social (CONEVAL)** en la página <https://www.coneval.org.mx/Medicion/IRS/Paginas/Rezago_social_AGEB_2010.aspx>.
...
Realiza la siguiente manipulación de datos para obtener la capa necesaria y guárdala en una nueva variable llamada `agebs_con_puntos_de_venta`:

1.  Toma como base la capa `agebs_con_datos`.
...
Es posible exportar este cuaderno también en formato *PDF*, sin embargo es necesario instalar antes alguna distribución de *LaTeX* desde <https://www.latex-project.org/get/>. Opcionalmente se puede instalar una versión reducida desde *R* usando la función `install_tinytex` del paquete *tinytex*.
...
-   Firke S. (2022). *Janitor*. Recuperado el 17 de junio, 2022 desde <http://sfirke.github.io/janitor/>.
