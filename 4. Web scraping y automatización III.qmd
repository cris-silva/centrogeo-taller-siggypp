...
![Ejemplo de una ficha de datos de inmueble.](Imagenes/ConsultaPublica.png){fig-alt="Ejemplo de una ficha de datos de inmueble." width="30%"}
...
![Identificación de secciones con el inspector web](Imagenes/InspectorWebPanel.png){fig-alt="Identificación de secciones con el inspector web" width="519"}
...
> Hasta este punto, podrías asumir que se pueden obtener las coordenadas del sitio extrayéndolas desde este URL y efectivamente, sería una buena estrategia para obtener datos georreferenciados. Sin embargo, si haces una revisión aleatoria de los registros encontrarás que el punto que se ilustra en el mapa de Google no coincide con la dirección que viene en la ficha, e incluso el mismo punto aparece en diferentes fichas. Esto puede ser una indicación de que, por alguna razón, los mapas de Google presentan información incorrecta. A pesar de esto, es conveniente conservar este dato para mantener la originalidad de la fuente y como referencia.
...
Almacena una capa de inmuebles históricos en una variable llamada `inmuebles_historicos`, realizando la siguiente manipulación de datos:

1.  Toma como base la tabla de datos completa `datos_acumulados` y adjúntale los datos de la tabla `datos_geocodificados` usando la función `left_join` para unir los registros donde coincide la columna `id`.
2.  Ahora que `datos_acumulados` ya tiene las variables de las coordenadas obtenidas de Here Maps, convierte la tabla `datos_geocodificados` en geodatos o simple features (sf) usando la función `st_as_sf` y los parámetros `coords = c("long", "lat")` para usar las variables `long` y `lat` que regresó el servicio de geocodificación como coordenadas, `crs = 4326` para especificar que se usa la proyección geográfica WGS-84 ([EPSG: 4326](https://epsg.io/4326)), `remove = FALSE` para conservar las columnas `long` y `lat` de las coordenadas y `na.fail = FALSE` para permitir que haya registros sin geometría (aquellos que no se pudieron geocodificar).
3.  Visualiza y explora el sf obtenido usando la función `glimpse`:

```{r Conversión de tabla en geodatos}
# Convertir la tabla de datos a sf
inmuebles_historicos <-
  datos_acumulados %>% # Tabla de datos completos
  left_join(datos_geocodificados, by = "id") %>% # Une la tabla con las coordenadas donde coincide el id
...
Guarda la capa `informacion_historica` en un archivo GeoPackage en la carpeta `Datos` usando la función `st_write` y especifica la extensión `.gpkg` para indicar que deseas usar este formato.

```{r Guardar GPKG}
# Guardar la capa como geopackage
st_write(inmuebles_historicos,
         str_c("Datos/inmuebles_historicos.gpkg"),
         delete_dsn = TRUE) # Sobreescribir si existe el archivo
```
...
Usa la función `slice_sample` con el parámetro `prop = 0.1` para tomar una muestra aleatoria del 10% de los puntos de la capa `inmuebles_historicos` y visualízalos en un mapa rápido usando la función `qtm`. Cambia el mapa base a *OpenStreetMap* para que tengas más información de referencia.
