...
Durante el transcurso de esta sesión, realizarás un ejercicio sencillo para extraer una tabla de datos económicos de una página de Wikipedia usando el paquete `rvest`, usarás Selenium para controlar el explorador Firefox y hacer una búsqueda en una página usando las funciones del paquete `RSelenium`, y convertirás una lista de direcciones en una capa geográfica usando el servicio web de Here*.*

Finalmente, aplicarás todos las técnicas anteriores para automatizar la extracción de datos de la página de consulta de puntos de venta de leche Liconsa y obtener una capa geográfica con sus ubicaciones, utilizando únicamente programación en R.

## Preparación del entorno

**Importante:** Para trabajar con este cuaderno, será necesario que tengas instalado el explorador Firefox que se puede instalar desde su [página oficial](https://www.mozilla.org).
...
El raspado web es una técnica ampliamente utilizada en el análisis de datos que permite extraer diversos elementos de una página web, como textos, imágenes y, sobre todo, tablas de datos. Esta herramienta se basa en la estructura de las páginas web, aprovechando el lenguaje de marcado HTML para identificar y extraer sus elementos de forma sencilla y automatizada.
...
La tabla que contiene la información sobre el PIB es la sexta. Selecciona la tabla correspondiente de la lista de `tablas` y guárdala en una nueva variable llamada `pib_mexico`:

1.  Toma como base el sexto elemento de la lista `tablas`.
2.  Visualiza la estructura de la tabla usando la función `glimpse`.

```{r Extraer tabla de PIB}
pib_mexico <-
  tablas[[6]] %>% # Usamos la quinta tabla de la página
  glimpse()
```

La tabla obtenida tiene nombres de variables que no son convenientes por que tienen mayúsculas y espacios, además las variables tienen un tipo de datos de texto. Antes de poder usar estos datos en un análisis es necesario hacer un proceso de limpieza, realiza la siguiente manipulación de datos y asigna el resultado a la variable `pib_mexico`:

1.  Toma como base la quinta tabla de `tablas`.
2.  Limpia los nombres de las variables usando la función `clean_names`.
3.  Convierte el tipo de datos de las variables (columnas) 2 a 7 a numérico aplicándoles la función `parse_numeric` y usando la función `mutate_at`.
4.  Divide entre 100 el valor de las variables (columnas) 4 a 7 aplicándoles la función `function(x) x/100` y usando la función `mutate_at`.
...
### Arranque del servidor de *Selenium* y conexión desde *R*

Antes de continuar, asegúrate de tener instalado Java en tu computadora. Si aún no lo tienes instalado, puedes descargar los instaladores de la versiones más recientes de JRE y JDK desde las página web <https://www.java.com/es/> y [https://www.oracle.com/java/technologies/downloads/](https://www.oracle.com/java/technologies/downloads/#jdk22-mac){.uri}, respectivamente.

El primer paso es iniciar un servidor de Selenium, existen varios métodos como puedes leer en la [documentación de RSelenium](https://docs.ropensci.org/RSelenium/articles/basics.html#do-i-need-to-run-a-selenium-server). En ésta práctica, iniciaremos el servidor usando la función `rsDriver` del paquete `RSelenium`, ya que no requiere de hacer instalaciones adicionales.

Para comenzar a enviar instrucciones a Selenium, debemos definir qué explorador vamos a manejar. Usa la función `rsDriver` y especifica el parámetro `browser = "firefox"` para indicar a Selenium que use Firefox. También debes agregar los parámetros `chromever = NULL`, `iedrver = NULL` y `phantomver = NULL` para indicar a Selenium que no intente abrir los otros exploradores. Guarda la instancia del servidor en la variable `servidor_selenium`:
...
Después de esta instrucción, el servidor de Selenium se inicia y una nueva ventana de Firefox debe abrirse automáticamente. A continuación, es necesario conectar el "control remoto" de R al servidor de Selenium que acabas de iniciar. Para ello, crea el `controlador` a partir del elemento de lista `client` de la variable `servidor_selenium` con la que arrancaste el servidor de Selenium en el paso anterior.

```{r Iniciar controlador de Selenium}
controlador <- servidor_selenium$client
```

A partir de este momento, ya es posible empezar a pasar instrucciones a través de `controlador` para manejar el explorador web usando las funciones del paquete RSelenium. Es recomendable que dividas en la pantalla las ventanas de RStudio y Firefox para que puedas ver cómo se ejecutan las siguientes instrucciones en el explorador.

### Envío de instrucciones a Firefox
...
Ordena a Firefox que envie la dirección de la sede de CDMX de CentroGeo (`"Contoy 137 Tlalpan Ciudad de México"`) como lista a través del método `sendKeysToElement` del control `caja_lugar`:

```{r Escribir en la caja de texto de lugar}
caja_lugar$sendKeysToElement(list("Contoy 137 Tlalpan Ciudad de México"))
```
...
Revisa el contenido de la página descargada. Abre el archivo `Datos/latlong_net.html` desde el panel Files de RStudio.

> ¿Se podría utilizar web scraping para obtener datos de este archivo?

Ahora, ordena a Firefox que abra la página de CentroGeo (<https://www.centrogeo.org.mx>) usando el método `navigate` de `controlador`:

```{r Abrir página web de CentroGeo}
controlador$navigate("https://www.centrogeo.org.mx")
```

Identifica el elemento de la liga del Directorio en el pie de la página, con el inspector y copia su dirección xPath. Crea una instancia de este elemento usando el método `findElement` con los parámetros `using = "xpath"` y la dirección que obtuviste del inspector `value = "/html/body/div[2]/footer[1]/div/div/div[2]/div/ul/li[1]/a"` y guarda el control en la variable `liga_directorio`:

```{r Identificar la liga a la página del directorio}
liga_directorio <- controlador$findElement(using = "xpath",
                                      value = "/html/body/div[2]/footer[1]/div/div/div[2]/div/ul/li[1]/a")
```

Ordena a Firefox que abra la liga:

```{r Abrir página de Posgrado}
liga_posgrado$clickElement()
```
...
Cierra todas las ventanas abiertas de Firefox y el servidor de Selenium, usando el método `closeAll` del `controlador`:

```{r Cerrar ventanas de Firefox}
controlador$closeall()
```

Detén el servidor de Selenium usando el método `stop()` en el elemento de lista `server` del `servidor_selenium`:

```{r Detener el servidor de Selenium}
servidor_selenium$server$stop()
```

En la parte final de la práctica volverás a iniciar el servidor para realizar el ejercicio.

## Geocodificación

La geocodificación es el proceso que nos permite obtener la ubicación geográfica a partir de direcciones y referencias en texto. Para llevar a cabo este proceso, podemos utilizar servicios web ofrecidos por plataformas como Google, OpenStreetMaps, Bing o Here, entre otros.

En este ejercicio, utilizaremos el paquete **`tidygeocoder`** para obtener una tabla con coordenadas a partir de una lista de direcciones correspondientes a las cuatro sedes de CentroGeo en el país.
...
Como vimos en clase, [hay diferentes servicios web que nos permiten geocodificar direcciones](https://jessecambon.github.io/tidygeocoder/articles/geocoder_services.html). Para este ejercicio, puedes usar la API de Bing por que se puede usar gratuitamente, tiene pocas limitaciones de uso y ofrece buenos resultados.

Para usar el servicio de Bing, es necesario crear una cuenta de usuario en <https://www.bingmapsportal.com> y obtener una llave de API (*token*). Ejecuta el siguiente *chunk* para definir en el entorno de R, la llave de API que generaste, asignándola a la variable `BINGMAPS_API_KEY` usando la función `Sys.setenv` como se indica a continuación:

```{r Definir la llave API de Bing}
# Establece tu llave API de Bing en cada nueva sesión de trabajo.
Sys.setenv("BINGMAPS_API_KEY" = "TU_API_KEY_AQUI")
```

> Ten cuidado con tus llaves API, no las compartas ni publiques en ningún sitio web para evitar que alguien más pueda hacer mal uso de ella. Puedes copiar la instrucción del *chunk* anterior y correrla en la consola si no deseas guardar tu llave en este cuaderno.

Ahora que ya puedes acceder al servicio web usando tu llave API envía la lista de direcciones al servicio de geocodificación de Bing para obtener una tabla con las coordenadas de las direcciones, realizando la siguiente manipulación de datos y guardando el resultado en la variable `sedes_centrogeo_geocodificadas`:

1.  Toma como base los datos de `sedes_centrogeo`.
2.  Envía los datos para geocodificar al servicio de Bing Maps usando la función `geocode`.
3.  Convierte la tabla resultante de la geocodificación en un objeto espacial *simple features (sf)* de puntos usando la función `st_as_sf` del paquete `sf` indicando cuáles son las columnas con las coordenadas con el parámetro `coords = c("long", "lat")` y el sistema de coordenadas geográficas WGS-84 (EPSG:4326) con el parámetro `crs = 4326.`
4.  Visualiza la estructura resultante usando la función `glimpse`.
...
sedes_centrogeo_geocodificadas <-
  ...
  geocode(address = "value",
          method = "bing") %>% 
  st_as_sf(coords = c("long", "lat"), # Convierte en sf indicando cuáles son las columnas delas coordenadas
           crs = 4326) %>% # Indica el sistema de coordenadas WGS-84
  glimpse()
```

Establece el modo de visualización interactiva del paquete `tmap` usando el parámetro `"view"` de la función `tmap_mode`. Visualiza en un mapa los resultados de la geocodificación, usa la función `qtm` para construir el mapa con los puntos de la capa `sedes_centrogeo_geocodificadas`:
...
qtm(sedes_centrogeo_geocodificadas)
```

El archivo `sedes_centrogeo_reales.gpkg` de la carpeta `Datos` contiene una capa con las ubicaciones reales de las sedes de CentroGeo. Ejecuta el siguiente *chunk* para comparar en el mapa las ubicaciones obtenidas por la geocodificación automática (puntos rojos) con las ubicaciones reales (puntos azules):

```{r Comparación de ubicaciones}
# Leer la capa de ubicaciones reales de las sedes
sedes_centrogeo_reales <- 
  st_read("Datos/sedes_centrogeo_reales.gpkg")

# Construir el mapa con la capa de ubicaciones reales y geocodificadas
tm_shape(sedes_centrogeo_reales) +
  tm_dots(size = 0.1, 
          col = "blue") + # Agregar la capa de ubicaciones reales en azul
  tm_shape(sedes_centrogeo_geocodificadas) +
  tm_dots(size = 0.1,
          col = "red") # Agregar la capa de ubicaciones geocodificadas en rojo
```

Explora el mapa y verifica la ubicación de los puntos geocodificados a través del API de Bing.

> ¿Las ubicaciones obtenidas automáticamente son correctas?
>
> Si encuentras diferencias, ¿a qué crees que se deben?
>
> ¿En qué casos usarías la geocodificación automática?
>
> ¿Cómo mejorarías la calidad de los resultados obtenidos?

## Caso práctico. Análisis de puntos de venta de Leche Liconsa (Parte I)

En esta sesión, prepararás los datos principales que se requieren para hacer una evaluación de la localización de puntos de venta de leche Liconsa en el país. Para ello es necesario obtener una capa geográfica con las ubicaciones de estos puntos de venta para poder compararlas o empezar a hacer análisis más específicos. Como no se cuenta con descarga directa de estos datos, será necesario extraer los datos de la página y geocodificarlos.
...
Antes de comenzar con éste proceso, es conveniente borrar todos los objetos del entorno. Presiona el botón *Clear objects from the workspace* (escoba) del panel *Environment* o ejecuta el siguiente bloque:

```{r Limpiar entorno para caso de uso}
rm(list = ls())
```

Inicia el servidor de Selenium usando la función `rsDriver` con los parámetros que usaste anteriormente y asígnalo a una nueva variable llamada `servidor_selenium`:. Incluye el parámetro `verbose = FALSE` para evitar mensajes de salida repetitivos.
...
Conecta a R con Firefox a través de Selenium creando un `controlador` usando el elemento de lista `[["client"]]` del `servidor_selenium`:
...
# Extraer sólo la tabla con los datos 
tabla_datos <-
...
Realiza un proceso de limpieza a los datos extraídos, aplicando la siguiente manipulación de datos:
...
# Cerrar las ventanas de Firefox y detener el servidor de Selenium:
controlador$closeall()
servidor_selenium$server$stop()
```
...
Usa las funciones del paquete `tidygeocoder` para realizar la geocodificación de las direcciones de puntos de venta de leche que extrajiste de la página oficial. Para este ejercicio utiliza el servicio de geocodificación de Here en vez del de Bing.

Para usar el servicio de Here, es necesario crear una cuenta de usuario en <https://platform.here.com> y obtener una llave de API (*token*). Ejecuta el siguiente *chunk* para definir en el entorno de R, la llave de API que generaste, asignándola a la variable `HERE_API_KEY` usando la función `Sys.setenv` como se indica a continuación:

```{r Definir la llave API de Here}
# Establece tu llave API de Bing en cada nueva sesión de trabajo.
...
2.  Usa la función `mutate` para agregar una nueva variable llamada `direccion` que contendrá la dirección en una sola línea concatenando las variables `calle`, `colonia`, `cp`, `municipio`, `estado` e incluye el texto `"MEXICO"` al final para indicar el país, usando la función `str_c`.
3.  Geocodifica las direcciones para obtener las coordenadas de los puntos de venta de leche, aplicando la usando la función `geocode` e indica que usarás la dirección concatenada con el parámetro `address = direccion` para geocodificar usando el servicio de Here con el parámetro `method = "here"`.
4.  Convierte la tabla resultante de la geocodificación en una capa de puntos usando la función `st_as_sf` del paquete `sf` indicando cuáles son las columnas con las coordenadas con el parámetro `coords = c("long", "lat")` y el sistema de coordenadas geográficas WGS-84 (EPSG:4326) con el parámetro `crs = 4326.`
5.  Visualiza el resultado de la geocodificación usando la función `glimpse`.

```{r Transformar tabla de direcciones de puntos de venta}
# Agregar una columna con la dirección en una sola línea, geocodificar y convertir en capa
puntos_venta_liconsa_geo <-
  puntos_venta_liconsa %>%
  mutate(direccion = str_c(calle, colonia, cp, municipio, estado, "MEXICO", sep = ", ")) %>%
...
Visualiza los datos geocodificados de `puntos_venta_liconsa_geo`, cambiando el modo de visualización como mapa web interactivo (`"view"`) usando la función `tmap_mode` y construye el mapa con la función `qtm`:

```{r Mapa de puntos de venta geocodificados}
# Cambiar a modo de visualización interactivo
tmap_mode("view")

# Visualizar en un mapa rápido
qtm(puntos_venta_liconsa_geo)
```

Revisa el mapa y comprueba la ubicación de algunas direcciones.

Guarda la capa geográfica `puntos_venta_liconsa_geo` como *shapefile* en el archivo `puntos_venta_liconsa.shp` en la carpeta `Datos` usando la función `st_write`:

```{r Guardar capa de puntos de venta}
puntos_venta_liconsa_geo %>% 
  st_write("Datos/puntos_venta_liconsa.shp",
