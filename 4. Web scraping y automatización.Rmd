...
La tabla que contiene la información sobre el PIB es la secta. Selecciona la tabla correspondiente de la lista de `tablas` y guárdala en una nueva variable llamada `pib_mexico`:
...
Identifica el elemento de la liga del menú Posgrado, con el inspector y copia su dirección CSS. Crea una instancia de este elemento usando el método `findElement` con los parámetros `using = "css selector"` y el la dirección que obtuviste del inspector `value = ".item-120 > a:nth-child(1)"` y guarda el control en la variable `liga_picig`:

```{r Identificar la liga a la página de Postgrado en CIG}
liga_posgrado <- controlador$findElement(using = "css selector",
                                      value = ".item-120 > a:nth-child(1)")
```
...
En el caso de este ejercicio, utilizaremos el paquete **`tidygeocoder`** para obtener una tabla con coordenadas a partir de una lista de direcciones correspondientes a las cuatro sedes de CentroGeo en el país.
...
sedes_centrogeo <-
  ...
  as_tibble()
# sedes_centrogeo <-
#   read_csv("Datos/sedes_centrogeo.csv") %>% 
#   glimpse() # Visualiza el contenido del archivo
sedes_centrogeo
```

CORREGIR

Se usa la API de Bing por que no tiene limitaciones de costo y uso y buenos resultados. comaprativa en <https://jessecambon.github.io/tidygeocoder/articles/geocoder_services.html> (Google cuesta y no tiene gratis)

Envía la lista de direcciones al servicio de geocodificación de *Bing Maps* para obtener una capa geográfica con las ubicaciones, realizando la siguiente manipulación de datos y guardando el resultado en la variable `sedes_centrogeo_geocodificadas`:

1.  Toma como base los datos de `sedes_centrogeo`.
2.  Envia los datos para geocodificar al servicio de *Bing Maps* usando la función `geocode`.
...
Sys.setenv("BINGMAPS_API_KEY" = "AuYj6J2QUZZa9C9aqubfCpVch18S4jVjiWs1MnZj_VfdAe8dcc9xme66p42lRS5w")
```
...
sedes_centrogeo_geocodificadas <-
  ...
  geocode(address = "value",
          method = "bing") %>% 
  st_as_sf(coords = c("long", "lat"), # Convierte en sf indicando cuáles son las columans delas coordenadas
           crs = 4326) %>% # Indica el sistema de coordenadas WGS-84
  glimpse()
# sedes_centrogeo_geocodificadas <-
#   sedes_centrogeo %>% 
#   geocode(street = "calle",
#           city = "ciudad",
#           county = "municipio",
#           state = "estado",
#           postalcode = "cp",
...
Establece el modo de visualización interactiva del paquete `tmap` usando el parámetro `"view"` de la función `tmap_mode`.

Visualiza en un mapa los resultados de la geocodificación, convirtiendo la tabla `sedes_centrogeo_geocodificadas` en un objeto espacial simple features de puntos usando la función `st_as_sf` del paquete `sf` indicando cuáles son las columnas con las coordenadas con el parámetro `coords = c("long", "lat")` y el sistema de coordenadas geográficas WGS-84 (EPSG:4326) con el parámetro `crs = 4326`, y finalmente usa la función `qtm` para construir el mapa:
...
Ejecuta el siguiente chunk para comparar en el mapa las ubicaciones obtenidas por la geocodificación automática (puntos rojos) con las ubicaciones reales (puntos azules):
...
Inicia el servidor de Selenium usando la función `rsDriver` con los parámetros que usaste anteriormente y asígnalo a una nueva variable llamada `servidor_selenium`:
